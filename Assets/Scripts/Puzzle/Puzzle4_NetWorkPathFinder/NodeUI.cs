...
public class NodeUI : MonoBehaviour
{
    ...
    public void Setup(int id, bool isActive, NodeType type = NodeType.Normal, int cost = 1, float timerDuration = 0f)
    {
        ...
        if (timerFillImage != null)
        {
            ...
            timerFillImage.gameObject.SetActive(type == NodeType.Timer);
        }
    }
    ...
    private Color GetNodeColor(bool isActive)
    {
        if (!isActive)
        {
            return nodeType == NodeType.Target ? new Color(0.7f, 0.3f, 0f) : disabledColor;
        }
    ...
    private void UpdateNodeIcon(bool isActive)
    {
        ...
        iconImage.gameObject.SetActive(nodeType != NodeType.Normal);
